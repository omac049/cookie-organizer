...
function isDomainRelated(cookieDomain, currentDomain) {
  // Implementation of domain relationship check
  if (!cookieDomain || !currentDomain) return false;
  
  // Check if domains match exactly
  if (cookieDomain === currentDomain) return true;
  
  // Check if cookie domain is parent of current domain
  if (currentDomain.endsWith(cookieDomain.charAt(0) === "." ? cookieDomain : "." + cookieDomain)) return true;
...
function categorizeCookie(cookie, siteUrl) {
  ...
  // Use isDomainRelated to help categorize the cookie
  const isRelated = isDomainRelated(cookie.domain, siteDomain);
  
  // Determine cookie relationship
  let relationshipCategory;
  if (cookieDomain === siteDomain || cookieDomain === siteDomain.replace(/^www\./, "") || cookieDomain === "www." + siteDomain.replace(/^www\./, "")) {
    relationshipCategory = "primary";
  } else if (cookieBaseDomain === siteBaseDomain || isRelated) {
...
async function scanActiveTabCookies(message, sendResponse) {
  try {
    console.log("Scanning active tab cookies, message:", message);
    ...
    try {
      // Parse the URL to validate it and extract domain if not provided
      const urlObj = new URL(tabUrl);
      if (!domain) {
        domain = urlObj.hostname;
      }
      console.log(`Scanning cookies for ${domain} (${tabUrl})`);
    } catch (parseError) {
      console.error("Error parsing URL:", parseError);
      sendResponse({
        success: false,
        error: "Invalid URL format. Please navigate to a valid website."
      });
      return;
    }
    ...
    try {
      ...
      if (!result.success) {
        console.error("Cookie organization failed:", result.error);
      } else {
      ...
      console.error("Error organizing cookies:", organizeError);
      sendResponse({
        success: false,
        error: "Failed to organize cookies: " + organizeError.message,
        originalError: organizeError.message
      });
    }
  } catch (error) {
    ...
    } else if (error.message.includes("blocked") || error.message.includes("firewall")) {
      errorMessage = "The scanner appears to be blocked by the website's security measures.";
    } else if (error.message.includes("headless")) {
      errorMessage = "The website is blocking our scanner. This can happen with some security configurations.";
    }
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  try {
    ...
    case "getCookiesForSite": 
      // Alternative action name for scanning cookies (for backward compatibility)
      console.log("Processing getCookiesForSite action for URL:", message.url);
      scanActiveTabCookies(message, sendResponse);
      return true; // Indicates we'll respond asynchronously
  ...
  try {
    ...
    chrome.cookies.remove({
      ...
      if (chrome.runtime.lastError) {
        ...
        chrome.cookies.set(newCookie, function(setCookieResult) {
